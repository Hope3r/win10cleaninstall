...
@REM :: Clear PageFile at shutdown and ActiveProbing
@REM ::reg add "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /v ClearPageFileAtShutdown /t REG_DWORD /d 1 /f > NUL 2>&1
@REM reg add "HKLM\SYSTEM\CurrentControlSet\Services\NlaSvc\Parameters\Internet" /v EnableActiveProbing /t REG_DWORD /d 0 /f > NUL 2>&1

:: Set Time to UTC
reg add "HKLM\SYSTEM\CurrentControlSet\Control\TimeZoneInformation" /v RealTimeIsUniversal /t REG_DWORD /d 1 /f > NUL 2>&1

@REM ::Disable Users On Login Screen
@REM reg add "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System" /v dontdisplaylastusername /t REG_DWORD /d 1 /f > NUL 2>&1

@REM ::Disable The Lock Screen
@REM reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\Personalization" /v NoLockScreen /t REG_DWORD /d 1 /f > NUL 2>&1
...
powercfg /h off
@REM :: Set Performance Plan to High Performance and display to never turn off
...
@REM @powershell -NoProfile -ExecutionPolicy Bypass -Command "choco install -y --force --allow-empty-checksums firefox thunderbird open-shell vlc 7zip jpegview vcredist-all directx onlyoffice wget"

@REM :: Configure Open-Shell
@REM :testos
@REM cls
@REM echo.
@REM echo :: Configuring Open-Shell
@REM echo.
@REM echo Due to restrictions with batch scripting it is required that you to 
@REM echo manually open the Open-Shell start menu for the first time.
@REM echo.
@REM echo Instructions:
@REM echo   1. Click Start
@REM echo   2. Click OK to close the Open-Shell Settings window
@REM echo   3. Open the Start Menu once more and then return to the CMD window
@REM echo.
@REM echo Press any key to continue:
@REM echo.
@REM pause > NUL

@REM set SHRTCT="%HOMEDRIVE%\Users\%username%\AppData\Roaming\OpenShell\Pinned\startscreen.lnk"
@REM if exist %SHRTCT% (
@REM 	del %HOMEDRIVE%\Users\%username%\AppData\Roaming\OpenShell\Pinned\startscreen.lnk /f > NUL 2>&1
@REM 	goto configureopenshell
@REM ) else (
@REM 	goto testos
@REM )

@REM :configureopenshell
@REM for /f "tokens=* USEBACKQ" %%i in (`wmic useraccount where "name="%username%"" get sid ^| findstr "S-"`) do set currentusername=%%i
@REM set currentusername=%currentusername:~0,-3%
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell" /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\OpenShell" /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\OpenShell\Settings" /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu" /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\ClassicExplorer" /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\ClassicExplorer\Settings" /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\ClassicExplorer" /v "ShowedToolbar" /t REG_DWORD /d 1 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\ClassicExplorer" /v "NewLine" /t REG_DWORD /d 0 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\ClassicExplorer\Settings" /v "ShowStatusBar" /t REG_DWORD /d 0 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu" /v "ShowedStyle2" /t REG_DWORD /d 1 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu" /v "CSettingsDlg" /t REG_BINARY /d c80100001a0100000000000000000000360d00000100000000000000 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu" /v "OldItems" /t REG_BINARY /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu" /v "ItemRanks" /t REG_BINARY /d 0 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\MRU" /v "0" /t REG_SZ /d "C:\Windows\regedit.exe" /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "Version" /t REG_DWORD /d 04040098 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "AllProgramsMetro" /t REG_DWORD /d 1 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "RecentMetroApps" /t REG_DWORD /d 1 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "StartScreenShortcut" /t REG_DWORD /d 0 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "SearchInternet" /t REG_DWORD /d 0 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "GlassOverride" /t REG_DWORD /d 1 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "GlassColor" /t REG_DWORD /d 0 /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "SkinW7" /t REG_SZ /d "Midnight" /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "SkinVariationW7 /t REG_SZ /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "SkinOptionsW7" /t REG_MULTI_SZ /d "USER_IMAGE=1"\0"SMALL_ICONS=0"\0"LARGE_FONT=0"\0"DISABLE_MASK=0"\0"OPAQUE=0"\0"TRANSPARENT_LESS=0"\0"TRANSPARENT_MORE=1"\0"WHITE_SUBMENUS2=0" /f > NUL 2>&1
@REM reg add "HKEY_USERS\%currentusername%\SOFTWARE\OpenShell\StartMenu\Settings" /v "SkipMetro" /t REG_DWORD /d 1 /f > NUL 2>&1
...
@REM SET LinkName=Firefox
@REM SET Esc_LinkDest=%%HOMEDRIVE%%\Users\%username%\AppData\Roaming\OpenShell\Pinned\!LinkName!.lnk
@REM SET Esc_LinkTarget=%%HOMEDRIVE%%\Program Files\Mozilla Firefox\Firefox.exe
...
@REM SETLOCAL ENABLEDELAYEDEXPANSION
@REM SET LinkName=Mozilla Thunderbird
@REM SET Esc_LinkDest=%%HOMEDRIVE%%\Users\user\AppData\Roaming\OpenShell\Pinned\!LinkName!.lnk
@REM SET Esc_LinkTarget=%%HOMEDRIVE%%\Program Files\Mozilla Thunderbird\Thunderbird.exe
@REM SET cSctVBS=CreateShortcut.vbs
@REM (
@REM   echo Set oWS = WScript.CreateObject^("WScript.Shell"^) 
@REM   echo sLinkFile = oWS.ExpandEnvironmentStrings^("!Esc_LinkDest!"^)
...
@REM del silent_installers.7z > NUL 2>&1
@REM del OldNewExplorerCfg.exe > NUL 2>&1
@REM del OldCalculatorforWindows10Cfg.exe > NUL 2>&1
@REM del hardentoolsCfg.exe > NUL 2>&1
@REM :: Download and configure OldNewExplorer
@REM cls
@REM echo.
@REM echo  :: Installing Third Party Programs
@REM echo.
@REM echo Downloading...
@REM wget -O silent_installers.7z https://wiki.ameliorated.info/lib/exe/fetch.php?media=silent_installers.7z > NUL 2>&1
@REM cls
@REM echo.
@REM echo  :: Installing Third Party Programs
@REM echo.
@REM echo Extracting...
@REM 7z x silent_installers.7z > NUL 2>&1
@REM cls
@REM echo.
@REM echo  :: Installing OldNewExplorer
@REM echo.
@REM echo Installing, please wait...
@REM start OldNewExplorerCfg.exe > NUL 2>&1
@REM timeout /t 15 /nobreak
@REM taskkill /f /im explorer.exe > NUL 2>&1
@REM taskkill /f /im OldNewExplorerCfg.exe > NUL 2>&1
@REM start OldNewExplorerCfg.exe > NUL 2>&1
@REM timeout /t 15 /nobreak
@REM taskkill /f /im OldNewExplorerCfg.exe > NUL 2>&1
@REM del OldNewExplorerCfg.exe > NUL 2>&1
@REM cls
@REM echo.
@REM echo  :: Installing Old Calculator for Windows 10
@REM echo.
@REM start OldCalculatorforWindows10Cfg.exe > NUL 2>&1
@REM timeout /t 10 /nobreak
@REM del OldCalculatorforWindows10Cfg.exe > NUL 2>&1
@REM cls
@REM echo.
@REM echo  :: Installing hardentools
@REM echo.
@REM start hardentoolsCfg.exe > NUL 2>&1
@REM timeout /t 60 /nobreak
@REM del hardentoolsCfg.exe > NUL 2>&1
@REM reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v ShowSuperHidden /t REG_DWORD /d 0 /f > NUL 2>&1

@REM goto reboot

:: Pls install mpv, it is better than vlc
